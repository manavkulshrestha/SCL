...
(from https://sdurobotics.gitlab.io/ur_rtde/guides/guides.html, implemented by Sam Rasp)"""
...
class RobotiqGripper:
    ...
    def suction_setup(self):
        if not self.is_active():
            self._reset()
            while not (self._get_var('ACT') == 0 and self._get_var('STA') == 0):
                time.sleep(0.01)

            self._set_var(self.ACT, 1)
            time.sleep(1.0)
            while not (self._get_var('ACT') == 1 and self._get_var('STA') == 3):
                time.sleep(0.01)
